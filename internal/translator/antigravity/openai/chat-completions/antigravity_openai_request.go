...
func ConvertOpenAIRequestToAntigravity(modelName string, inputRawJSON []byte, _ bool) []byte {
	...
	// Apply thinking configuration: convert OpenAI reasoning_effort or Claude thinking.* to Gemini CLI thinkingConfig.
	...
	} else if tc := gjson.GetBytes(rawJSON, "thinkingConfig"); tc.Exists() && tc.IsObject() {
		// Support Claude-format thinking parameter: thinking.type + thinking.budget_tokens
		// Support Gemini native thinkingConfig format (top-level, camelCase):
		// { "thinkingConfig": { "thinkingBudget": 20000, "includeThoughts": true } }
		// Also supports snake_case variants (thinking_budget, thinking_level, include_thoughts).
		hasConfig := false
		if b := tc.Get("thinkingBudget"); b.Exists() && b.Type == gjson.Number {
			out, _ = sjson.SetBytes(out, thinkingPath+".thinkingBudget", int(b.Int()))
			hasConfig = true
		} else if b := tc.Get("thinking_budget"); b.Exists() && b.Type == gjson.Number {
			out, _ = sjson.SetBytes(out, thinkingPath+".thinkingBudget", int(b.Int()))
			hasConfig = true
		}
		if l := tc.Get("thinkingLevel"); l.Exists() && l.Type == gjson.String {
			out, _ = sjson.SetBytes(out, thinkingPath+".thinkingLevel", l.String())
			hasConfig = true
		} else if l := tc.Get("thinking_level"); l.Exists() && l.Type == gjson.String {
			out, _ = sjson.SetBytes(out, thinkingPath+".thinkingLevel", l.String())
			hasConfig = true
		}
		if hasConfig {
			if inc := tc.Get("includeThoughts"); inc.Exists() {
				out, _ = sjson.SetBytes(out, thinkingPath+".includeThoughts", inc.Bool())
			} else if inc := tc.Get("include_thoughts"); inc.Exists() {
				out, _ = sjson.SetBytes(out, thinkingPath+".includeThoughts", inc.Bool())
			} else {
				out, _ = sjson.SetBytes(out, thinkingPath+".includeThoughts", true)
			}
		}
	} else if t := gjson.GetBytes(rawJSON, "thinking"); t.Exists() && t.IsObject() {
		// Support Claude-format thinking parameter: thinking.type + thinking.budget_tokens
		thinkingType := t.Get("type").String()
		if thinkingType == "enabled" {
			if b := t.Get("budget_tokens"); b.Exists() && b.Type == gjson.Number {
				budget := int(b.Int())
				out, _ = sjson.SetBytes(out, thinkingPath+".thinkingBudget", budget)
				out, _ = sjson.SetBytes(out, thinkingPath+".includeThoughts", true)
			} else {
				// type=enabled without budget_tokens -> auto mode
				out, _ = sjson.SetBytes(out, thinkingPath+".thinkingBudget", -1)
				out, _ = sjson.SetBytes(out, thinkingPath+".includeThoughts", true)
			}
	...
	} else if maxCompTok := gjson.GetBytes(rawJSON, "max_completion_tokens"); maxCompTok.Exists() && maxCompTok.Type == gjson.Number {
		out, _ = sjson.SetBytes(out, "request.generationConfig.maxOutputTokens", maxCompTok.Num)
	}
			...
			if (role == "system" || role == "developer") && len(arr) > 1 {
				...
				if content.Type == gjson.String {
					...
					for _, item := range items {
						// Skip Claude-specific cache_control field (not supported by Antigravity upstream)
