...
func (e *AntigravityExecutor) buildRequest(ctx context.Context, auth *cliproxyauth.Auth, token, modelName string, payload []byte, stream bool, alt, baseURL string) (*http.Request, error) {
	...
	if strings.Contains(modelName, "claude") {
		// Only add toolConfig when tools are actually present in the request.
		// Sending toolConfig without tools causes upstream 400 INVALID_ARGUMENT.
		// Reference: antigravity2api-nodejs common.js buildRequestBody() only adds toolConfig when hasTools=true.
		hasTools := gjson.Get(payloadStr, "request.tools").Exists() && len(gjson.Get(payloadStr, "request.tools").Array()) > 0
		if hasTools {
		...
		// Claude models don't support topP when thinking is enabled (upstream returns 400).
		// Reference: antigravity2api-nodejs parameterNormalizer.js toGenerationConfig()
		thinkingEnabled := strings.Contains(modelName, "-thinking") ||
			gjson.Get(payloadStr, "request.generationConfig.thinkingConfig.includeThoughts").Bool()
		if thinkingEnabled {
			payloadStr, _ = sjson.Delete(payloadStr, "request.generationConfig.topP")
		}
	} else {
